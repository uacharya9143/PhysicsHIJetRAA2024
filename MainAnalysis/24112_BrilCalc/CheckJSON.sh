...
#$brilcalc lumi --normtag /cvmfs/cms-bril.cern.ch/cms-lumi-pog/Normtags/normtag_BRIL.json -i ${Input} -o temp.csv #uttam disabled for now
$brilcalc lumi   -i ${Input} -o temp.csv #uttam disabled for now

#BrilValue=`grep -v "^#" temp.csv | cut -d ',' -f 6 | Sum`

BrilValue=$(grep -v "^#" temp.csv | cut -d ',' -f 6 | awk '{sum += $1} END {print sum}') #disabled by uttam for now

# Set your fraction here: e.g., used 1 out of total 60 files
USED_PARTS=1
TOTAL_PARTS=60
...
#DHRemove "$DH" Lumi "${Tag/Fine}_PHYSICS"
